...
async function checkCompStructure() {
  // Try Atlanta which we know works
  const address = "123 Peachtree St, Atlanta, GA 30303";
  ...
  console.log("=== COMP STRUCTURE ANALYSIS (Atlanta) ===\n");
  
  if (data.payload?.comps && data.payload.comps.length > 0) {
    const comp = data.payload.comps[0];
    console.log("Number of comps:", data.payload.comps.length);
  ...
  if (data.payload?.stats?.future?.metrics) {
    ...
    console.log("All months:", data.payload.stats.future.metrics.map(m => `${m.date}: occ=${(m.occupancy*100).toFixed(0)}%, adr=$${m.adr.toFixed(0)}, rev=$${m.revenue.toFixed(0)}`));
  }
}
