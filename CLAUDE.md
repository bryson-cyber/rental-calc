...
- **Every app files with doola directly** (`DOOLA_API_BASE_URL` is hard-pinned to api.doola.com / api.test.doola.com). No app submits filings through another app, and none exposes an endpoint that accepts filings from a sibling. **Whop is a PAYMENTS rail only** where enabled (HFO members pay their retail/state fee through Whop embedded checkout) — the legacy Whop FILING leg is explicit opt-in (LLC_PROVIDER=whop) and never used for new filings; `LLC_PROVIDER` fails safe to doola when unset (2026-09-05).
