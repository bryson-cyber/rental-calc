...
export const generateRentalReportPdf = (data: ReportData): void => {
  const { property, market, bedroom_performance } = data;
  ...
  doc.text(
    `${propertyInfo.bedrooms} BR • ${propertyInfo.bathrooms} BA • Sleeps ${propertyInfo.accommodates}${market ? ` • ${market.market_name} Market` : ''}`,
    margin,
    43
  );
  ...
  if (market) {
    ...
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    doc.text(`${market.market_name} Market Overview`, margin, yPos);
    yPos += 12;
  ...
  if (comps && comps.length > 0) {
    ...
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    doc.text('Top Performing Properties Nearby', margin, yPos);
    yPos += 10;

    const compData = comps.slice(0, 8).map((comp, idx) => [
      `#${idx + 1}`,
