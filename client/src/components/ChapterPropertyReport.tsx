...
function ChapterNav({ chapters, activeChapter, onChapterClick }: {
  ...
  return (
    <div className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-[#1A1A1A]/10 py-4">
...
function ChapterSection({ id, title, children }: {
  ...
  return (
    <section id={id} className="scroll-mt-24 py-12 border-b border-[#1A1A1A]/10 last:border-0">
      <motion.div
      ...
      <table className="w-full">
        <thead>
          <tr className="border-b-2 border-[#D4A84B]/30">
            {headers.map((header, idx) => (
              <th
                key={idx}
                className={`py-3 px-4 text-sm font-semibold text-[#1A1A1A] ${
        ...
        <tbody>
          {rows.map((row, rowIdx) => (
            <tr
              key={rowIdx}
              className={`border-b border-[#1A1A1A]/5 transition-colors ${
                highlight === rowIdx ? 'bg-[#D4A84B]/10' : 'hover:bg-[#1A1A1A]/5'
              }`}
            >
              {row.map((cell, cellIdx) => (
                <td
                  key={cellIdx}
                  className={`py-4 px-4 ${
                    cellIdx === 0 ? 'text-left font-medium text-[#1A1A1A]' : 'text-right text-[#1A1A1A]/70'
                  }`}
                >
                  {cell}
                </td>
...
function ThoughtProcess({ children }: { children: React.ReactNode }) {
  return (
    <div className="mt-6 p-4 bg-[#D4A84B]/10 rounded-xl border border-[#D4A84B]/30">
      <div className="flex items-start gap-3">
        <Lightbulb className="w-5 h-5 text-[#D4A84B] flex-shrink-0 mt-1" />
        <div>
          <p className="font-semibold text-[#1A1A1A]">The Thought Process</p>
          <p className="text-sm text-[#1A1A1A]/70 mt-1">{children}</p>
        </div>
      </div>
    </div>
  );
        ...
        <div className="container mx-auto px-4 py-12">
          ...
          <div className="max-w-3xl">
            <div className="flex items-center gap-3 mb-4">
              <Home className="w-6 h-6 text-[#D4A84B]" />
              <span className="text-sm font-medium text-[#D4A84B] uppercase tracking-wider">
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <ChapterSection id="chapter-1" title="1. First, We Look at the Property Itself">
            ...
            <div className="grid md:grid-cols-2 gap-4 mb-6">
              <div className="bg-white rounded-xl p-4 shadow-md border-l-4 border-[#D4A84B]">
                <div className="flex items-center gap-3 mb-2">
                  <Bed className="w-5 h-5 text-[#D4A84B]" />
                  <span className="font-semibold text-[#1A1A1A]">{property.bedrooms} Bedrooms</span>
                </div>
              ...
              <div className="bg-white rounded-xl p-4 shadow-md border-l-4 border-[#D4A84B]">
                <div className="flex items-center gap-3 mb-2">
                  <Bath className="w-5 h-5 text-[#D4A84B]" />
                  <span className="font-semibold text-[#1A1A1A]">{property.bathrooms} Bathrooms</span>
                </div>
                <p className="text-sm text-[#1A1A1A]/60">
              ...
              <div className="bg-white rounded-xl p-4 shadow-md border-l-4 border-green-500">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="font-semibold text-[#1A1A1A]">Sleeps {property.accommodates}</span>
                </div>
                <p className="text-sm text-[#1A1A1A]/60">
              ...
              <div className="bg-white rounded-xl p-4 shadow-md border-l-4 border-blue-500">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="font-semibold text-[#1A1A1A]">{property.city} Location</span>
                </div>
                <p className="text-sm text-[#1A1A1A]/60">
          ...
          <ChapterSection id="chapter-2" title="2. Next, We Analyze the Local Market">
                ...
                <DataTable
                  ...
                  rows={[
                    [
                      ...
                      <span key="top10-val" className="font-bold text-[#D4A84B]">{formatCurrency(revenue_percentiles.p90)}</span>,
            ...
            <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
              ...
              {monthly_forecast && monthly_forecast.length > 0 && (
                <div className="mb-8">
                  <p className="text-sm text-[#1A1A1A]/60 mb-4">Revenue & Occupancy by Month (Green = Above Average, Orange = Below Average)</p>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                ...
                <div className="p-4 bg-[#D4A84B]/10 rounded-lg">
                  <p className="text-sm text-[#D4A84B] font-medium">Annual Total</p>
                  <p className="text-xl font-bold text-[#1A1A1A]">{formatCurrency(revenue_estimate.annual)}</p>
                </div>
              </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[#1A1A1A]/10">
                      <th className="py-2 px-3 text-left font-medium text-[#1A1A1A]/60">Month</th>
                      <th className="py-2 px-3 text-right font-medium text-[#1A1A1A]/60">Revenue</th>
                      <th className="py-2 px-3 text-right font-medium text-[#1A1A1A]/60">Occupancy</th>
                    </tr>
                  </thead>
                  <tbody>
          ...
          <ChapterSection id="chapter-3" title="3. Then, We Study the Competition">
            ...
            <div className="bg-[#D4A84B]/10 border border-[#D4A84B]/30 rounded-xl p-6 mb-8">
            ...
            {displayComps && displayComps.length > 0 && (
              <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
                <p className="text-sm text-[#1A1A1A]/60 mb-4">Top 15 Competitors by Annual Revenue (Green = Meets 2x Rule, Red = Below Threshold)</p>
            ...
            <div className="bg-white rounded-xl p-4 shadow-sm mb-6">
              <div className="flex flex-wrap items-center gap-4">
                <div className="flex items-center gap-2">
                  <label className="text-sm font-medium text-[#1A1A1A]/70">Property Type:</label>
                  <select
                    value={propertyTypeFilter}
                    onChange={(e) => setPropertyTypeFilter(e.target.value)}
                    className="px-3 py-1.5 rounded-lg border border-[#1A1A1A]/20 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-[#D4A84B]/50"
                  >
                ...
                <div className="flex items-center gap-2">
                  <label className="text-sm font-medium text-[#1A1A1A]/70">Min Rating:</label>
                  <select
                ...
                {(propertyTypeFilter !== 'all' || minRatingFilter > 0) && (
                  <button
                    ...
                    className="text-sm text-[#D4A84B] hover:text-[#D4A84B]/80 font-medium"
                  >
                    Clear Filters
                  </button>
                )}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              <div className="bg-white rounded-xl p-4 shadow-sm">
                <p className="text-sm text-[#1A1A1A]/60">Total Same-BR Listings</p>
                <p className="text-2xl font-bold text-[#1A1A1A]">{displayComps.length}</p>
              </div>
              <div className="bg-white rounded-xl p-4 shadow-sm">
                <p className="text-sm text-[#1A1A1A]/60">Meet 2x Threshold</p>
                <p className="text-2xl font-bold text-green-600">{displayComps.filter(c => c.annual_revenue >= minRevenueThreshold).length}</p>
              </div>
              <div className="bg-white rounded-xl p-4 shadow-sm">
                <p className="text-sm text-[#1A1A1A]/60">Avg Revenue</p>
                <p className="text-2xl font-bold text-[#1A1A1A]">{formatCurrency(displayComps.reduce((sum, c) => sum + c.annual_revenue, 0) / displayComps.length || 0)}</p>
              </div>
              <div className="bg-white rounded-xl p-4 shadow-sm">
                <p className="text-sm text-[#1A1A1A]/60">Top Performer</p>
                <p className="text-2xl font-bold text-[#D4A84B]">{formatCurrency(displayComps[0]?.annual_revenue || 0)}</p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg overflow-hidden mb-8">
              ...
              <div className="divide-y divide-[#1A1A1A]/5 max-h-[800px] overflow-y-auto">
                {displayComps.map((comp, idx) => (
                // Show all top performers, highlight those meeting threshold
                  <div key={comp.id} className="p-4 hover:bg-[#1A1A1A]/5 transition-colors">
                    <div className="flex items-start gap-4">
                      ...
                      {(comp.image_url || getAirbnbImageUrl(comp.airbnb_url)) ? (
                        ...
                        <div className="flex-shrink-0 w-24 h-20 bg-[#1A1A1A]/10 rounded-lg flex items-center justify-center">
                          <Home className="w-8 h-8 text-[#1A1A1A]/30" />
                        </div>
                      )}
                      
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="w-6 h-6 bg-[#D4A84B]/20 rounded-full flex items-center justify-center text-xs font-bold text-[#D4A84B]">
                            {idx + 1}
                          </span>
                          ...
                          {comp.airbnb_url ? (
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="font-semibold text-[#1A1A1A] hover:text-[#D4A84B] transition-colors flex items-center gap-1"
                            >
                              {comp.title}
                              <ExternalLink className="w-3 h-3" />
                            </a>
                          ) : (
                            <span className="font-semibold text-[#1A1A1A]">{comp.title}</span>
                          )}
                        </div>
                        <p className="text-sm text-[#1A1A1A]/60 mb-2">
                          ...
                          {comp.rating && (
                            <span className="ml-2 inline-flex items-center gap-1">
                              <Star className="w-3 h-3 fill-[#D4A84B] text-[#D4A84B]" />
          ...
          <ChapterSection id="chapter-4" title="4. Finally, We Project the Profit">
              ...
              <DataTable
                ...
                rows={[
                  [
                    ...
                    <span key="startup-val" className="font-bold text-[#1A1A1A]">{formatCurrency(estimatedStartupCosts)}</span>,
              ...
              <DataTable
                ...
                rows={[
                  ...
                  [
                    ...
                    <span key="total-val" className="font-bold text-[#1A1A1A]">{formatCurrency(totalMonthlyExpenses)}</span>,
                ...
                rows={[
                  ...
                  [
                    <span key="realistic" className="flex items-center gap-2">
                      <Target className="w-4 h-4 text-[#D4A84B]" />
            ...
            <div className="mt-12 bg-white rounded-2xl shadow-lg p-8">
              ...
              <div className="bg-[#1A1A1A]/5 rounded-xl p-6">
                <p className="text-center text-[#1A1A1A]/80">
                  ...
                  <span className="text-[#D4A84B] font-bold"> {formatCurrency(optimisticRevenue - conservativeRevenue)}</span> per year. 
            ...
            {/* CTA Section */}
            <div className="mt-8 bg-gradient-to-br from-[#1A1A1A] to-[#1e293b] rounded-2xl p-8 text-white">
              <div className="max-w-2xl mx-auto text-center">
                ...
                <div className="grid md:grid-cols-3 gap-4 mb-8 text-left">
                  <div className="bg-white/10 rounded-lg p-4">
                    <CheckCircle2 className="w-5 h-5 text-[#D4A84B] mb-2" />
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  <button className="bg-[#D4A84B] text-[#1A1A1A] px-8 py-4 rounded-xl font-semibold hover:bg-[#d4b876] transition-colors flex items-center justify-center gap-2">
          ...
          <ChapterSection id="chapter-5" title="5. Setting Up for Success">
            ...
            <div className="bg-white rounded-2xl shadow-lg overflow-hidden mb-8">
              <div className="bg-gradient-to-r from-[#D4A84B] to-amber-500 p-4">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    <h4 className="font-semibold text-[#1A1A1A] mb-4 flex items-center gap-2">
                      <Home className="w-4 h-4 text-[#D4A84B]" />
                      Property Selection
                    </h4>
                    <div className="space-y-3">
                      <div className="flex items-start gap-3 py-2 border-b border-[#1A1A1A]/10">
                        <CheckCircle2 className="w-5 h-5 text-green-500 flex-shrink-0 mt-0.5" />
                        <span className="text-[#1A1A1A]/70">Location analysis (walkability, attractions, parking)</span>
                      </div>
                      <div className="flex items-start gap-3 py-2 border-b border-[#1A1A1A]/10">
                        <CheckCircle2 className="w-5 h-5 text-green-500 flex-shrink-0 mt-0.5" />
                        <span className="text-[#1A1A1A]/70">Lease negotiation for STR approval</span>
                      </div>
                      <div className="flex items-start gap-3 py-2 border-b border-[#1A1A1A]/10">
                        <CheckCircle2 className="w-5 h-5 text-green-500 flex-shrink-0 mt-0.5" />
                        <span className="text-[#1A1A1A]/70">Market demand validation</span>
                      </div>
                      <div className="flex items-start gap-3 py-2">
                        <CheckCircle2 className="w-5 h-5 text-green-500 flex-shrink-0 mt-0.5" />
                        <span className="text-[#1A1A1A]/70">Revenue potential verification</span>
                      </div>
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-start gap-3 py-2 border-b border-[#1A1A1A]/10">
                        <CheckCircle2 className="w-5 h-5 text-green-500 flex-shrink-0 mt-0.5" />
                        <span className="text-[#1A1A1A]/70">Interior design that photographs beautifully</span>
                      </div>
                      <div className="flex items-start gap-3 py-2 border-b border-[#1A1A1A]/10">
                        <CheckCircle2 className="w-5 h-5 text-green-500 flex-shrink-0 mt-0.5" />
                        <span className="text-[#1A1A1A]/70">Furniture selection for durability & style</span>
                      </div>
                      <div className="flex items-start gap-3 py-2 border-b border-[#1A1A1A]/10">
                        <CheckCircle2 className="w-5 h-5 text-green-500 flex-shrink-0 mt-0.5" />
                        <span className="text-[#1A1A1A]/70">Professional photography & virtual tour</span>
                      </div>
                      <div className="flex items-start gap-3 py-2">
                        <CheckCircle2 className="w-5 h-5 text-green-500 flex-shrink-0 mt-0.5" />
                        <span className="text-[#1A1A1A]/70">Amenity packages guests love</span>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="mt-6 bg-[#D4A84B]/10 rounded-xl p-4 border border-[#D4A84B]/30">
                  <p className="text-[#1A1A1A] text-center">
                    <strong>The Secret:</strong> Top performers invest in <span className="text-[#D4A84B] font-bold">professional setup</span> from day one. 
            ...
            <div className="bg-white rounded-2xl shadow-lg overflow-hidden mb-8">
              ...
              <div className="p-6">
                <div className="space-y-4">
                  <div className="flex items-start gap-4 py-3 border-b border-[#1A1A1A]/10">
                    <div className="bg-blue-100 rounded-full p-2">
                      <Star className="w-5 h-5 text-blue-600" />
                    </div>
                    <div>
                      <span className="font-medium text-[#1A1A1A]">Professional Photography</span>
                      <p className="text-sm text-[#1A1A1A]/60">Listings with pro photos get 40% more bookings and can charge 20% higher rates</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-4 py-3 border-b border-[#1A1A1A]/10">
                    <div className="bg-blue-100 rounded-full p-2">
                    ...
                    <div>
                      <span className="font-medium text-[#1A1A1A]">Compelling Description</span>
                      <p className="text-sm text-[#1A1A1A]/60">Story-driven copy that sells the experience, not just the space</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-4 py-3 border-b border-[#1A1A1A]/10">
                    <div className="bg-blue-100 rounded-full p-2">
                    ...
                    <div>
                      <span className="font-medium text-[#1A1A1A]">Strategic Pricing</span>
                      <p className="text-sm text-[#1A1A1A]/60">Launch pricing strategy to build reviews quickly, then optimize for revenue</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-4 py-3">
                    ...
                    <div>
                      <span className="font-medium text-[#1A1A1A]">Must-Have Amenities</span>
                      <p className="text-sm text-[#1A1A1A]/60">Fast WiFi, smart TV, quality linens, coffee station, and local guidebook</p>
                    </div>
                  </div>
                </div>
            ...
            {/* Our Setup Process */}
            <div className="bg-white rounded-2xl shadow-lg overflow-hidden mb-8">
              <div className="bg-gradient-to-r from-[#D4A84B] to-amber-600 p-4">
                <h3 className="text-lg font-semibold text-white flex items-center gap-2">
              ...
              <div className="p-6">
                <div className="grid md:grid-cols-4 gap-4">
                  <div className="text-center">
                    <div className="bg-[#D4A84B]/10 rounded-full w-16 h-16 flex items-center justify-center mx-auto mb-3">
                      <span className="text-2xl font-bold text-[#D4A84B]">1</span>
                    </div>
                    <h4 className="font-semibold text-[#1A1A1A] mb-2">Market Analysis</h4>
                    <p className="text-sm text-[#1A1A1A]/70">We analyze the market, find the best property, and negotiate the lease</p>
                  </div>
                  <div className="text-center">
                    <div className="bg-[#D4A84B]/10 rounded-full w-16 h-16 flex items-center justify-center mx-auto mb-3">
                      <span className="text-2xl font-bold text-[#D4A84B]">2</span>
                    </div>
                    <h4 className="font-semibold text-[#1A1A1A] mb-2">Design & Furnish</h4>
                    <p className="text-sm text-[#1A1A1A]/70">Professional interior design, furniture procurement, and complete setup</p>
                  </div>
                  <div className="text-center">
                    <div className="bg-[#D4A84B]/10 rounded-full w-16 h-16 flex items-center justify-center mx-auto mb-3">
                      <span className="text-2xl font-bold text-[#D4A84B]">3</span>
                    </div>
                    <h4 className="font-semibold text-[#1A1A1A] mb-2">Launch & Optimize</h4>
                    <p className="text-sm text-[#1A1A1A]/70">Professional photos, listing creation, and pricing strategy setup</p>
                  </div>
                  <div className="text-center">
                    <div className="bg-[#D4A84B]/10 rounded-full w-16 h-16 flex items-center justify-center mx-auto mb-3">
                      <span className="text-2xl font-bold text-[#D4A84B]">4</span>
                    </div>
                    <h4 className="font-semibold text-[#1A1A1A] mb-2">Go Live</h4>
                    <p className="text-sm text-[#1A1A1A]/70">Your property goes live and starts earning within 2-4 weeks</p>
                  </div>
                </div>
            ...
            {/* Setup Comparison */}
            <div className="bg-white rounded-2xl shadow-lg overflow-hidden mb-8">
              <div className="bg-gradient-to-r from-[#1A1A1A] to-[#1e293b] p-4">
                <h3 className="text-lg font-semibold text-white flex items-center gap-2">
              ...
              <div className="p-6">
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="border-2 border-[#1A1A1A]/20 rounded-xl p-6">
                    <h4 className="text-xl font-bold text-[#1A1A1A] mb-4 text-center">DIY Setup</h4>
                    <div className="space-y-3">
                      <div className="flex justify-between py-2 border-b border-[#1A1A1A]/10">
                        <span className="text-[#1A1A1A]/70">Revenue Potential</span>
                        <span className="font-medium">{formatCurrency(conservativeRevenue)}</span>
                      </div>
                      <div className="flex justify-between py-2 border-b border-[#1A1A1A]/10">
                        <span className="text-[#1A1A1A]/70">Time to Launch</span>
                        <span className="font-medium text-amber-600">2-3 months</span>
                      </div>
                      <div className="flex justify-between py-2 border-b border-[#1A1A1A]/10">
                        <span className="text-[#1A1A1A]/70">Design Quality</span>
                        <span className="font-medium text-amber-600">Variable</span>
                      </div>
                      <div className="flex justify-between py-2 border-b border-[#1A1A1A]/10">
                        <span className="text-[#1A1A1A]/70">Listing Optimization</span>
                        <span className="font-medium text-amber-600">Learning curve</span>
                      </div>
                      <div className="flex justify-between py-2">
                        <span className="text-[#1A1A1A]/70">Photography</span>
                        <span className="font-medium text-amber-600">Self or basic</span>
                      </div>
                    </div>
                  </div>
                  <div className="border-2 border-[#D4A84B] rounded-xl p-6 bg-[#D4A84B]/5">
                    <h4 className="text-xl font-bold text-[#1A1A1A] mb-4 text-center flex items-center justify-center gap-2">
                ...
                <div className="mt-6 bg-[#D4A84B]/10 rounded-xl p-4 border border-[#D4A84B]/30">
                  <p className="text-center text-[#1A1A1A]">
                    <strong>The Difference:</strong> Professional setup typically generates <span className="text-[#D4A84B] font-bold">20-35% higher revenue</span> from day one. 
