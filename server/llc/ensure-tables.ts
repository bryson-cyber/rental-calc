...
export async function ensureLlcTables(): Promise<void> {
  ...
  try {
    ...
    await db.execute(sql`
      CREATE TABLE IF NOT EXISTS \`llc_required_actions\` (
        \`id\` int AUTO_INCREMENT NOT NULL,
        \`registrationId\` int NOT NULL,
        ...
        INDEX \`llc_required_action_registration_idx\` (\`registrationId\`),
        INDEX \`llc_required_action_open_idx\` (\`open\`,\`registrationId\`),
        INDEX \`llc_required_action_company_idx\` (\`doolaCompanyId\`)
      )
    `);
