...
## CRITICAL Bug: API Returning HTML Instead of JSON (Jan 20, 2026)

- [ ] Error: "Unexpected token '<', "<!doctype "... is not valid JSON"
- [ ] Affects: Zip code search, auto-search in Map View
- [ ] Root cause: Server returning HTML error page instead of JSON response
- [ ] Investigate: Check if endpoint exists, check server logs


## Bug Fixes (Jan 20, 2026)

### Step 5 Map View API Errors Fix:
- [x] Fixed tRPC API call errors in MapViewContent component
- [x] Replaced raw fetch() calls with proper tRPC client queries (trpcUtils.client.compData.getListings.query())
- [x] Added safety checks for Google Maps AdvancedMarkerElement availability
- [x] Verified map loads correctly with property markers and auto-population from Step 1
- [x] Tested full flow: Enter address → Set Property → See on Map → Map displays with 6 comparable properties
